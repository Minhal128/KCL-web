    ...
    const fetchContent = async () => {
        try {
            ...
            const [featuredRes, recentRes, trendingRes, genresRes, watchlistRes] = await Promise.all([
                contentAPI.getFeaturedContent(10).catch(err => ({ data: { data: [] } })),
                contentAPI.getAllContent({ page: 1, limit: 10 }).catch(err => ({ data: { data: { content: [] } } })),
                contentAPI.getAllContent({ page: 1, limit: 10, isFeatured: true }).catch(err => ({ data: { data: { content: [] } } })),
                contentAPI.getAllGenres().catch(err => ({ data: { data: [] } })),
                watchlistAPI.getWatchlist().catch(err => ({ data: { wishlist: [] } }))
            ]);
